...
这份 skill 负责把 Harness Engineering 映射到“本地智能体”仓库的文档入口、模块落点、实现顺序和验证路径上。

它不表示当前仓库已经在所有 Harness 层面达到产品级成熟态。默认判断口径应是：当前主干是否已成立，哪些层已经有骨架或基础能力，哪些层仍处于收口和补强阶段。

使用这份 skill 时，目标是基于现有主干持续收紧上下文、工具、权限、验证、artifact、记忆、事件和长任务交接，而不是把任何已有骨架直接表述为“该层已经完全成熟”。

如果问题落在 `评估与观测` 或 `恢复机制`，应额外显式区分：当前仓库是否只有 verify、confirmation、handoff 等基础能力，还是已经具备监控、归因、重试、回滚和不可恢复错误处理等更完整的运行时治理能力。
