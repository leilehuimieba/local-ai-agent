use runtime_core::{
    ModelRef, ProviderRef, RunEvent, RunRequest, WorkspaceRef, get_observations, persist_lifecycle_observations,
    search_observations,
};
use serde_json::json;
...
fn sample_request() -> RunRequest {
    let workspace_id = format!("workspace-m3-get-{}", unix_millis());
    RunRequest {
        request_id: "mem-15-request".to_string(),
        run_id: "run-m3".to_string(),
        session_id: "session-m3".to_string(),
        trace_id: "trace-m3".to_string(),
        user_input: "get observations".to_string(),
...
fn main() {
    ...
    let search = search_observations(&request, "details", 3);
    let ids = search.items.iter().map(|item| item.observation_id).collect::<Vec<_>>();
    let report = get_observations(&request, &ids, 3);
    println!(
        "{}",
        serde_json::to_string_pretty(&report).expect("get report should serialize")
    );
}
