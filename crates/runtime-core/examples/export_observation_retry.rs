use runtime_core::{ModelRef, ProviderRef, RunEvent, RunRequest, WorkspaceRef, run_observation_retry_flow};
use serde_json::json;
...
fn sample_request() -> RunRequest {
    let workspace_id = format!("workspace-m2-retry-{}", unix_millis());
    RunRequest {
        request_id: "mem-10-request".to_string(),
        run_id: "run-m2".to_string(),
        session_id: "session-m2".to_string(),
        trace_id: "trace-m2".to_string(),
        user_input: "retry flow report".to_string(),
        ...
        context_hints: queue_fail_hints(),
...
fn main() {
    let report = run_observation_retry_flow(&sample_request(), &sample_events());
    println!(
        "{}",
        serde_json::to_string_pretty(&report).expect("retry report should serialize")
    );
}
