...
fn sample_request() -> RunRequest {
    let workspace_id = format!("workspace-m2-{}", unix_millis());
    RunRequest {
        request_id: "mem-09-request".to_string(),
        run_id: "run-m2".to_string(),
        session_id: "session-m2".to_string(),
        trace_id: "trace-m2".to_string(),
        user_input: "queue flow report".to_string(),
...
fn main() {
    let report = run_observation_queue_flow(&sample_request(), &sample_events());
    println!(
        "{}",
        serde_json::to_string_pretty(&report).expect("queue flow report should serialize")
    );
}
