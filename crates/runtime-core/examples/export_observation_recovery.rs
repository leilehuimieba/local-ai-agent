...
fn sample_request() -> RunRequest {
    let workspace_id = format!("workspace-m2-recovery-{}", unix_millis());
    RunRequest {
        request_id: "mem-11-request".to_string(),
        run_id: "run-m2".to_string(),
        session_id: "session-m2".to_string(),
        trace_id: "trace-m2".to_string(),
        user_input: "recovery flow report".to_string(),
...
fn main() {
    ...
    let first = run_observation_queue_flow(&request, &sample_events());
    let second = run_observation_queue_flow(&request, &sample_events());
    let report = json!({
        "first_run": first,
        "recovery_run": second,
        "recovered": second.failed_count == 0 && second.processed_count == 6
    });
    println!(
        "{}",
        serde_json::to_string_pretty(&report).expect("recovery report should serialize")
    );
}
