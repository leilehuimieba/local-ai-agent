...
func fillLogEntryOutcome(entry *contracts.LogEntry, event contracts.RunEvent) {
	...
	entry.DetailPreview = pickFirst(event.DetailPreview, event.Metadata["detail_preview"], event.ResultSummary)
	entry.RawOutputRef = pickFirst(event.RawOutputRef, event.Metadata["raw_output_ref"], event.ArtifactPath)
	...
	entry.ActivityState = pickFirst(event.ActivityState, event.Metadata["activity_state"])
	entry.HeartbeatAt = pickFirst(event.HeartbeatAt, event.Metadata["heartbeat_at"])
	entry.StallSeconds = pickFirst(event.StallSeconds, event.Metadata["stall_seconds"])
	entry.WaitingReason = pickFirst(event.WaitingReason, event.Metadata["waiting_reason"])
	entry.NextActionHint = pickFirst(event.NextActionHint, event.Metadata["next_action_hint"])
	entry.FailureRoute = pickFirst(event.FailureRoute, event.Metadata["failure_route"])
	entry.UpdatedAt = pickFirst(event.UpdatedAt, event.Metadata["updated_at"])
	entry.TaskTitle = pickFirst(event.TaskTitle, event.Metadata["task_title"])
	entry.ActiveTool = pickFirst(event.ActiveTool, event.Metadata["active_tool"], entry.ToolName)
	entry.EvidenceRef = pickFirst(event.EvidenceRef, event.Metadata["evidence_ref"])
...
func normalizeEventFields(event contracts.RunEvent, previous contracts.RunEvent) contracts.RunEvent {
	...
	event.DetailPreview = pickFirst(event.DetailPreview, event.Metadata["detail_preview"], event.ResultSummary, previous.DetailPreview)
	event.RawOutputRef = pickFirst(event.RawOutputRef, event.Metadata["raw_output_ref"], event.ArtifactPath, previous.RawOutputRef)
	event.ActivityState = pickFirst(event.ActivityState, event.Metadata["activity_state"], previous.ActivityState)
	event.HeartbeatAt = pickFirst(event.HeartbeatAt, event.Metadata["heartbeat_at"], previous.HeartbeatAt)
	event.StallSeconds = pickFirst(event.StallSeconds, event.Metadata["stall_seconds"], previous.StallSeconds)
	event.WaitingReason = pickFirst(event.WaitingReason, event.Metadata["waiting_reason"], previous.WaitingReason)
	event.NextActionHint = pickFirst(event.NextActionHint, event.Metadata["next_action_hint"], event.Metadata["next_step"], previous.NextActionHint)
	event.FailureRoute = pickFirst(event.FailureRoute, event.Metadata["failure_route"], previous.FailureRoute)
	event.UpdatedAt = pickFirst(event.UpdatedAt, event.Metadata["updated_at"], event.Timestamp, previous.UpdatedAt)
	event.TaskTitle = pickFirst(event.TaskTitle, event.Metadata["task_title"], previous.TaskTitle, event.Summary)
	event.ActiveTool = pickFirst(event.ActiveTool, event.Metadata["active_tool"], event.ToolName, previous.ActiveTool)
	event.EvidenceRef = pickFirst(event.EvidenceRef, event.Metadata["evidence_ref"], previous.EvidenceRef)
...
func normalizeLogEntry(item contracts.LogEntry) contracts.LogEntry {
	...
	item.DetailPreview = pickFirst(item.DetailPreview, item.Metadata["detail_preview"], item.ResultSummary)
	item.RawOutputRef = pickFirst(item.RawOutputRef, item.Metadata["raw_output_ref"], item.ArtifactPath)
	item.ActivityState = pickFirst(item.ActivityState, item.Metadata["activity_state"])
	item.HeartbeatAt = pickFirst(item.HeartbeatAt, item.Metadata["heartbeat_at"])
	item.StallSeconds = pickFirst(item.StallSeconds, item.Metadata["stall_seconds"])
	item.WaitingReason = pickFirst(item.WaitingReason, item.Metadata["waiting_reason"])
	item.NextActionHint = pickFirst(item.NextActionHint, item.Metadata["next_action_hint"], item.Metadata["next_step"])
	item.FailureRoute = pickFirst(item.FailureRoute, item.Metadata["failure_route"])
	item.UpdatedAt = pickFirst(item.UpdatedAt, item.Metadata["updated_at"], item.Timestamp)
	item.TaskTitle = pickFirst(item.TaskTitle, item.Metadata["task_title"], item.Summary)
	item.ActiveTool = pickFirst(item.ActiveTool, item.Metadata["active_tool"], item.ToolName)
	item.EvidenceRef = pickFirst(item.EvidenceRef, item.Metadata["evidence_ref"])
