...
func ProviderSettingsView(
	...
	if credentialOK && SavedConfigPending(credential, runtimeRecord, runtimeOK) {
		return credentialProviderConfig(provider, credential)
	}
	if runtimeOK && runtimeRecord.Status == "applied" {
		return runtimeProviderConfig(provider.ProviderID, runtimeRecord)
	}
	if credentialOK {
		return credentialProviderConfig(provider, credential)
	}
...
func runtimeProviderConfig(
	providerID string,
	runtimeRecord state.RuntimeProviderRecord,
) config.ProviderConfig {
	return config.ProviderConfig{
		ProviderID: providerID, DisplayName: runtimeRecord.DisplayName, BaseURL: runtimeRecord.BaseURL,
