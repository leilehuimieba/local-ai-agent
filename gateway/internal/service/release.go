...
func RunReleaseStep(repoRoot string, req ReleaseRunRequest) (ReleaseRunResult, error) {
	spec, err := releaseSpec(repoRoot, req.Step)
	if err != nil {
		return ReleaseRunResult{}, err
	}
	return executeReleaseSpec(repoRoot, req.Step, spec)
}
...
func executeReleaseSpec(repoRoot string, step string, spec releaseCommandSpec) (ReleaseRunResult, error) {
	started := time.Now()
