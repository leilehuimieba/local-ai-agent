...
- 最近更新时间：2026-04-13
- 状态：进行中（待提审）
- 当前阶段：见 `docs/11-hermes-rebuild/current-state.md`（本文件仅记录该 change 的任务进展）
- 已完成：
  ...
  - 调查层事件状态别名已清理：`EventTimeline` 状态映射先消费失败/待确认/完成明确信号，再回退默认分类，减少事件类型同义映射漂移。
  - 记录页状态徽标映射已补齐：`HistoryTimelineSection` 状态映射先消费失败/待确认/完成明确信号，并统一以状态键驱动徽标与卡片 tone。
  - 主线程状态卡密度已二次压缩：`chat-panel-simple` 下状态卡头部与正文间距、字号和行高收紧，首屏可见条目数提升且不改状态语义。
  - 任务页移动端布局压测已完成：窄屏下修复工具条换行、调查层切换按钮点击区、状态卡挤压问题，任务页移动端可读性提升。
  - 状态词典收口阶段验收包已补齐：新增 `artifacts/主题45-状态词典收口阶段验收包.md`，沉淀覆盖页、映射点、回归结果、已知遗留与下一阶段基线。
  - 记录页结构化筛选已增强：筛选栏新增“结果/风险/验证/治理”分组条，并联动详情栏区块自动定位，支持一键复查。
  - 设置页诊断链路可读性已增强：诊断区改为“当前状态/影响范围/建议动作”三组结果卡，并统一接入状态词典短词与语义色。
  - 前端阶段性回归脚本清单已补齐：新增 `scripts/run-frontend-wave1-regression.ps1` 与 `artifacts/主题48-前端阶段性回归脚本清单.md`，固定自动构建与人工检查项入口。
  - 阶段性提审包已补齐：新增 `review.md`，集中记录主题 38-48 提审范围、证据入口、关键提交、风险与回退。
  - 已完成接口样本字段复核落盘：`tmp/stage-e-audit-consumption/latest.json`。
  - 已完成最小构建回归：`frontend/` 下 `npm run build` 通过。
  - 已完成回归脚本留证：`tmp/frontend-wave1-regression/latest.json`。
- 进行中：
  - 主题 38-48 已完成收口，当前进入提审与归档决策窗口。
- 阻塞点：无。
- 下一步：
  - 按回归脚本清单补一轮截图级人工复核留证。
  - 在确认后将本 change 迁入 `changes/archive/<日期>/` 或切换为下一波主题池入口。
