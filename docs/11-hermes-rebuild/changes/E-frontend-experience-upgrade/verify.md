...
1. 记录页筛选分组可见
   - 筛选栏新增“结果 / 风险 / 验证 / 治理”分组入口，可按复查目标一键筛选时间线。
2. 分组筛选逻辑可用
   - “结果”分组命中结果输出，“风险”分组命中错误与确认，“验证”分组命中验证摘要，“治理”分组命中治理与沉淀信号。
3. 筛选与详情联动可见
   - 选择分组后，详情栏自动定位到对应区块（结果=摘要、风险=风险与错误、验证=验证与后续、治理=关键 Metadata）。
4. 最小构建回归
   - 在 `frontend/` 目录执行 `npm run build`，`tsc -b && vite build` 通过。
...
1. 诊断结果三段分组可见
   - 设置页“诊断与导出”模块改为三组结果卡：`当前状态 / 影响范围 / 建议动作`，可按分组快速扫读。
2. 分组状态词典语义一致
   - 三组结果卡与模块徽标均基于统一状态词典输出“进行中 / 待确认 / 完成 / 失败”，不再使用并行词汇。
3. 建议动作链路可读性增强
   - 建议动作按“运行时可达性、错误、警告、复检动作”顺序输出，支持从诊断结论直接落到下一步处置。
4. 最小构建回归
   - 在 `frontend/` 目录执行 `npm run build`，`tsc -b && vite build` 通过。
