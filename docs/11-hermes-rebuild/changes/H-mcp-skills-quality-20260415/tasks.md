...
| H03-40 | 详细样本回填两轮回填与来源用尽确认 | 验证/文档 | done | 依赖 H03-39，由主控要求继续收口缺口时启动 | 已完成两轮回填（skill-hit-effective-calibration → long-tail/representative-coverage），所有 11 份专项证据文件逐一核查，确认无可直接追溯的新增结构化来源；`latest.json`、`gap 文档`、`status.md` 已同步更新准确缺口 | 不代表缺口已补齐，不代表继续整理即可发现新来源，不代表 H-03 已 ready | 只用于把“缺口还有多少”从模糊推测推进到“来源已穷尽”的精确口径 | `formal-batch-detail-backfill-gap-20260423.md`, `tmp/stage-h-mcp-skills/latest.json`, `status.md` |
