...
for ($i = 1; $i -le $Rounds; $i++) {
  ...
  $roundsDetail += [ordered]@{
    ...
    confirm_boundary_stage_matched = $confirm.after_confirmation.boundary_stage_matched
    confirm_boundary_next_step_matched = $confirm.after_confirmation.boundary_next_step_matched
    retry_boundary_stage_matched = $retry.retry_run.boundary_stage_matched
...
$confirmBoundaryStageCount = @($roundsDetail | Where-Object { $_.confirm_boundary_stage_matched }).Count
$confirmBoundaryNextStepCount = @($roundsDetail | Where-Object { $_.confirm_boundary_next_step_matched }).Count
$retryBoundaryStageCount = @($roundsDetail | Where-Object { $_.retry_boundary_stage_matched }).Count
...
$summary = [ordered]@{
  ...
  confirm_boundary_stage_count = $confirmBoundaryStageCount
  confirm_boundary_next_step_count = $confirmBoundaryNextStepCount
  retry_boundary_stage_count = $retryBoundaryStageCount
  confirm_boundary_stage_rate = [Math]::Round($confirmBoundaryStageCount / $Rounds, 4)
  confirm_boundary_next_step_rate = [Math]::Round($confirmBoundaryNextStepCount / $Rounds, 4)
  retry_boundary_stage_rate = [Math]::Round($retryBoundaryStageCount / $Rounds, 4)
